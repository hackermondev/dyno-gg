...
export default class Player extends Base {
	...
	public connect(): Promise<eris.VoiceConnection> {
		...
		return this.client.joinVoiceChannel(this.voiceChannel.id).then((player: any) => {
			...
			this.nodeHost = `${player.node.host}`;
			this.dyno.internalEvents.emit('music', { type: 'start', guild: this.guild, channel: this.voiceChannel })

			this.metrics.server.timeStarted = new Date();
			...
			this.player.on('error', async (err: string) => {
				...
				const leave = this.queue.size > 1 ? false : true;
				this.stop(leave);

				return new Promise((res: any, rej: any) =>
					setTimeout(() => this.play().then(res).catch(rej), 500));
			});
			...
			this.player.on('disconnect', (err?: string) => {
				...
				this.dyno.internalEvents.emit('music', { type: 'disconnect',  guild: this.guild, channel: this.voiceChannel });
			});
		});
	}
	...
	public async add(guildConfig: dyno.GuildConfig, options: PlayOptions) {
		...
		if (guildConfig.isPremium === true && isLink === true && tracks.length > 1) {
			this.prom.register.getSingleMetric('dyno_app_music_playlists').inc();
			...
			this.prom.register.getSingleMetric('dyno_app_music_adds').inc();
	...
	public async play(guildConfig?: dyno.GuildConfig, options?: PlayOptions) {
		...
		if (this.voiceChannel.voiceMembers.size === 1 && this.voiceChannel.voiceMembers.has(this.client.user.id)) {
		...
		if (options && options.search) {
			...
			this.dyno.internalEvents.emit('music', { type: 'search', guild: this.guild, channel: this.voiceChannel, search: options.search, trackInfo: trackInfo })
		...
		this.player.once('end', (data: any) => {
			...
			this.queue.shift().then((queue: QueueItems) => {
				if (queue !== undefined && queue.length > 0) {
					this.dyno.internalEvents.emit('music', { type: 'changeSong', guild: this.guild, channel: this.voiceChannel, trackInfo: trackInfo })
		...
		this.dyno.internalEvents.emit('music', { type: 'playSong', guild: this.guild, channel: this.voiceChannel })
	...
	public async skip(message?: eris.Message, guildConfig?: dyno.GuildConfig, force?: boolean): Promise<any> {
		...
		this.dyno.internalEvents.emit('music', { type: 'skip', guild: this.guild, channel: this.voiceChannel })
	...
	public async playQueueItem(guildConfig: dyno.GuildConfig, index: string|number): Promise<any> {
		...
		if(this.player.isPlaying) {
			this.dyno.internalEvents.emit('music', { type: 'playQueueItem', guild: this.guild, channel: this.voiceChannel, index })
		}
	...
	public destroy(): void {
		...
		this.dyno.internalEvents.emit('music', { type: 'end',  guild: this.guild, channel: this.voiceChannel });
	}
