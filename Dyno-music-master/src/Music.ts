...
export default class Music extends Module {
	...
	constructor(dynoInstance: dyno.Dyno) {
		...
		this.metrics = new MetricsManager(dynoInstance);
	}
	...
	public async getPlayerCount(): Promise<any> {
		let vcs;
		...
		return Object.values(vcs).reduce((a: any, b: string) => {
			a += parseInt(b, 10);
			return a;
		}, 0);
	}
	...
	public async voiceChannelLeave({ member, channel, guild, guildConfig }: any) : Promise<void> {
		if(this.dyno.isReady) {
			if(this.dyno.players.has(guild.id) && this.dyno.players.get(guild.id).playing) {
				this.dyno.internalEvents.emit('music', { type: 'leave', user: member, guild, channel });
			}
		}
	}
	...
	public async voiceChannelJoin({ member, channel, guild, guildConfig }: VoiceJoinEvent): Promise<void> {
		if(this.dyno.isReady) {
			if(this.dyno.players.has(guild.id) && this.dyno.players.get(guild.id).playing) {
				this.dyno.internalEvents.emit('music', { type: 'join', user: member, guild, channel });
			}
		}

		if (!this.dyno.isReady || !this.config.isPremium || !guildConfig.isPremium || member.bot) {
			return;
		}
		...
		if (guildConfig.music != undefined && guildConfig.music.autojoinChannel != undefined) {
			...
			const player = this.dyno.players.get(guild.id);
	...
	public async toggleRepeat(message: eris.Message, guildConfig: any): Promise<any> {
		...
		guildConfig.music.repeat = guildConfig.music.repeat !== false ? false : true;
	...
	private updateStats(): void {
		...
		this.prom.register.getSingleMetric('dyno_app_voice_total').set({ state }, players)
		this.prom.register.getSingleMetric('dyno_app_voice_playing').set({ state }, playingCount)
	...
	private connectionCheck(): void {
		...
		each([...voicePlayers.values()], (voicePlayer: any) => {
			...
			if (channel == undefined || !this.dyno.players.has(voicePlayer.guildId)) {
				voicePlayer.stop();
				this.client.leaveVoiceChannel(voicePlayer.channelId);
				return;
			}

			const player: Player = this.dyno.players.get(voicePlayer.guildId);
