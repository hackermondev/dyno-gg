...
export default class Tags extends Module {
	...
	public async createOrEdit(message: eris.Message, tag: string, content: string, guildConfig: dyno.GuildConfig) {
	...
	public async listTags(message: eris.Message, guildId: string, guildConfig: dyno.GuildConfig) {
		const guild = (<eris.GuildChannel>message.channel).guild;
		if (!this.isEnabled(guild, this, guildConfig)) {
			return Promise.reject(null);
		}
