...
class Server extends Controller {
	...
	async server(bot, req, res) {
		...
		// stats variables
		res.locals.categories = channels.filter(c => c.type === 4).map(c => Object.create(c));
		...
		if (guildConfig.autoroles && guildConfig.autoroles.roleOnJoin) {
			res.locals.roleOnJoin = res.locals.roles.find(r => r.id === guildConfig.autoroles.roleOnJoin);
		}

		if (guildConfig.autoroles && guildConfig.autoroles.autoroles) {
			res.locals.autoroles = guildConfig.autoroles.autoroles.map(a => {
				const autorole = Object.create(a);
				const role = res.locals.roles.find(r => r.id === a.role);
				autorole.role = role ? role : { id: a.role, name: 'Deleted Role' };
				return autorole;
			});
		}
