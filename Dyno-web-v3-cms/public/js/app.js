...
(function ($) {
	...
	$(document).ready(function() {
		...
		$('.queue-control').on('click', function (e) {
			...
			$('.tab-control').removeClass('is-active');
			$('.tab-content').removeClass('is-active');
			$('.tab-control[href="#/module-music"]').addClass('is-active');
			$('.subtab-control[href="#/music-queue"]').closest('li').addClass('is-active');
			$tab.addClass('is-active');
		});
		...
		$('.command').on('change', function () {
			var command = $(this).val(),
				enabled = this.checked,
				url = '/api/server/' + server + '/toggleCommand',
		...
		// subcommand checkboxes
		$('.subcommand').on('change', function () {
			var command = $(this).val(),
				enabled = this.checked,
				url = '/api/server/' + server + '/toggleSubcommand',
				data = { command: command, enabled: enabled };

			var [cmd, subcmd] = command.split('.');

			apiPost(url, data, function (err, msg) {
				if (err) return showError('An error occurred.');
				var enabledOrDisabled = enabled ? 'enabled' : 'disabled';
				return showSuccess("Command '" + cmd + '.' + subcmd + "' has been " + enabledOrDisabled + '.');
			});
		});
