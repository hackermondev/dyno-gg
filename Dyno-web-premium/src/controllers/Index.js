...
class Index extends Controller {
	...
	async staffserverlist(bot, req, res) {
		...
		if (!req.session.isAdmin) {
			const globalConfig = await db.models.Dyno.findOne({}, { listingAccess: 1 }).lean();
	...
	invite(bot, req, res) {
		res.locals.redirectURI = `https://discordapp.com/oauth2/authorize?client_id=${config.client.id}&scope=bot%20identify%20guilds&response_type=code&redirect_uri=https://dyno.gg/return&permissions=${config.defaultPermissions}`;
	...
	async team(bot, req, res) {
		try {
			const globalConfig = await db.models.Dyno.findOne({}, { team: 1 }).lean().exec();
