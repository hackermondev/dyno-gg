...
export default class SettingSelector extends React.Component {
	...
	saveSettings = () => {
		...
		const { ignoredChannels, ignoredRoles } = this.state;
		updateFilterSettings(setting, { ignoredChannels, ignoredRoles }, this.props.label);
	}
		...
		return (
			<div className='automod-selector rich-select control rich-toggle'>
				...
				<Modal open={this.state.settingsModalOpen} classNames={modalClasses} little={true} onClose={this.closeSettings}>
					...
					<button className='button is-success' onClick={this.saveSettings}>Save</button>
				</Modal>
			</div>
		)
	}
