...
export default class CommandModal extends React.Component {
    ...
    render() {
		...
		const responseChannelOptions = channels.filter(c => c.type === 0).map(c => ({ value: c.id, label: c.name }));
		...
		const extraResponseOptions = JSON.parse(JSON.stringify(responseChannelOptions));
		extraResponseOptions.unshift({ value: 'commandChannel', label: '[Current Channel]' });
		extraResponseOptions.unshift({ value: 'userChannel', label: '[DM Current User]' });
									...
									{formData.responses && formData.responses.length ? (
											formData.responses.filter(r => r.type === 'message').map(response => (
												<div key={response.id} id={response.id} className='control'>
													<RichSelect
														// text='Channel'
														defaultValue={response.channel ? extraResponseOptions.find(o => o.value === response.channel) : false}
														defaultOption='Select Channel'
														options={extraResponseOptions}
