...
const logger = require('../../core/logger').get('AutomodLogs');
const db = require('../../core/models');

class AutomodLogs extends Controller {
	constructor(bot) {
		super(bot);

		const basePath = '/api/modules/:id/automodlogs';
	...
	getLogs(guildId, page = 0, pageSize = 10) {
        ...
        return db.collection('automodlogs')
                .find({ guild: guildId })
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
