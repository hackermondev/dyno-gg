...
export default class Status extends React.Component {
	...
	onServerIDChange = (event) => {
		this.setState({ serverId: event.target.value });
	}

	onServerIDEnter = async (e) => {
		if (e.key !== 'Enter') {
			return;
		}
		...
		console.log(e.key, serverId);

		if (!serverId || !serverId.length) {
			return;
		}

		const statusRequest = await axios.get(`/api/status/${serverId}`);
				...
				<div className="columns server-finder">
					<div className="find-server column is-half is-full-mobile">
						<p className="control has-addons">
							<input className="input" type="text" onChange={this.onServerIDChange} onKeyDown={this.onServerIDEnter} placeholder="Look up your server (by ID)" />
						</p>
					</div>
					{foundInfo}
				</div>
