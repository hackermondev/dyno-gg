...
class MemberCount extends Command {
	...
	async execute({ message, args }) {
		...
		if (guild.memberCount === guild.members.size) {
			fields.push({ name: 'Humans', value: guild.members.filter(m => !m.bot).length.toString(), inline: true });
			fields.push({ name: 'Bots', value: guild.members.filter(m => m.bot).length.toString(), inline: true });
