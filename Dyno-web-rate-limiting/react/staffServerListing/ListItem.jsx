...
export default class ListItem extends React.Component {
    ...
    componentDidMount() {
        ...
        if (this.props.server) {
            this.setState({
                ...
                homepage: this.props.server.homepage || false,
            });
        }
    }
    ...
    toggleOption = async (field, event) => {
        try {
            const data = {};
            const stateData = {};
            stateData[field] = event.target.checked ? true : false;

            data.field = field;
            data.value = event.target.checked ? true : false;

            await axios.post(`/api/serverlist/${this.props.server.id}/toggleOption`, data);
            ...
            <div className="server-list-item-wrapper" style={style}>
                ...
                <div className="staff-controls">
                    ...
                    <input
                        ...
                        onChange={this.toggleOption.bind(this, 'homepage')} />
                    Home</label>
                </div>
            </div>
        );
